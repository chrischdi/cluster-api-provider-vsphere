...
func (c *MachineLogCollector) machineIPAddresses(ctx context.Context, ctrlClient client.Client, m *clusterv1.Machine) ([]string, error) {
	...
	if m.Spec.InfrastructureRef.GroupVersionKind().Group == vmwarev1.GroupVersion.Group {
		...
		key, err := virtualMachineObjectKey(m.Name, m.Namespace, vsphereMachine.Spec.NamingStrategy)
		if err != nil {
			return nil, errors.Wrapf(err, "generating VirtualMachine name")
		}
...
func virtualMachineObjectKey(machineName, machineNamespace string, namingStrategy *vmwarev1.VirtualMachineNamingStrategy) (*client.ObjectKey, error) {
	name, err := vmoperator.GenerateVirtualMachineName(machineName, namingStrategy)
	if err != nil {
		return nil, err
	}
