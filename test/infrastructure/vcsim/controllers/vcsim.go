...
func createVMTemplate(ctx context.Context, vCenter *vcsimv1.VCenter) error {
	...
	if vCenter.Spec.Model != nil {
		datacenters = pointer.IntDeref(vCenter.Spec.Model.Datacenter, simulator.VPX().Datacenter) // VPX is the same base model used when creating vcsim
	}
